...
/** 멤버관리 페이지 주소 */
const MEMBER_MANAGE_URL =
    'https://cafe.naver.com/ManageWholeMember.nhn?clubid=30130324';
...
/** 크롤링 실행할 함수 */
const main = async (pageNumber = 1) => {
    const page = await naverLogin();
    ...
    for (let i = 0; i < pageNumber; i++) {
        ...
        const data = await page.evaluate(async () => {
            ...
            userList.forEach((list) => {
                ...
                const email =
                    list.textContent.split('(')[1].replace(/[{()}]/g, '') +
                    '@naver.com';
            ...
            /** 그리고 다음 페이지 버튼으로 클릭! */
            const nextPageBtn = links.item(
                nextPage % 12 === 0 ? 1 : nextPage % 12
            );
...
main(60).catch((e) => console.error(e));
